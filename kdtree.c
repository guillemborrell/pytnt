	      ...
	      double *focus,int NDIST, double *distances)
{
  #pragma omp parallel
  {
    ...
    vt = calloc(NPOINTS, sizeof(struct kd_node_t));
    
    /* Assign the voxels to the structure */
    counter = 0;
    for (i = 0; i < 3*NPOINTS; i+=3){
      vt[counter].x[0] = voxels[i];
      vt[counter].x[1] = voxels[i+1];
      vt[counter].x[2] = voxels[i+2];
      counter++;
    }
    ...
    for (i = 0; i < NDIST; i++){
      ...
      found = 0;
      nearest(root, &ft, 0, 3, &found, &distances[i]);
    }
    free(vt);
  }
  return 0;
